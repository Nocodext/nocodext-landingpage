              ...
              <TabsList className="mr-4 whitespace-nowrap h-10 flex items-center gap-2 bg-transparent">
                {tabOptions.map((tab) => (
                  <TabsTrigger 
                    ...
                    className="flex items-center gap-2 bg-white font-open-sans py-2 bg-opacity-50 text-black hover:bg-nocodext hover:text-white rounded-md transition-colors data-[state=active]:bg-nocodext data-[state=active]:text-white"
                  >
                    {tab.icon}
                    <span>{tab.label}</span>
                  </TabsTrigger>
